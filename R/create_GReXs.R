...
create_GReXs = function(X_file, exp_files, contexts, out_dir, snps, gene_name, decomposition_dir = NULL, context_thresh = 3, alpha = 0.5, num_folds = 10, run_CxC = FALSE){
  ...
  if(run_CxC){
    ...
    names(Yhats_cxc) = names(Ys_cxc)
  }
  
  message("Finished cross-validation. Calculating r2 performance metrics")
  
  names(Yhats_tiss)<-names(Ys)
  names(Yhats_full)<-names(Ys)
  ...
  for(i in 1:q){
    ...
    if(!all(is.na(Yhats_het[[i]]))){
      ...
      Yhats_full[[i]][rownames(full_values), ]<- full_values$pred
  ...
  save(Yhats_het, Yhats_hom, Yhats_full, Yhats_tiss, file = paste0(out_dir,gene_name,"_crossval_predictors"))
...
#################### added this #################
Yhats_full<-vector("list", q)

for(i in 1:q){
  Yhats_het[[i]]<-matrix(NA, ncol=1, nrow=lengths_y[i], 
                         dimnames = list(rownames_y[[i]], "pred"))
  Yhats_hom[[i]]<-matrix(NA, ncol=1, nrow=lengths_y[i], 
                         dimnames = list(rownames_y[[i]], "pred"))
...
end.time.universal = Sys.time()
time.taken.univeral.setup <- end.time.universal-start.time.universal


start.time.content.universal = Sys.time()
shared.total.time = 0
specific.total.time = 0
gbat.total.time = 0
...
for(cur_fold in 1:num_folds){
  ...
  shared.end.time = Sys.time()
  cur_shared_time = shared.end.time - shared.start.time
  shared.total.time = shared.total.time + cur_shared_time
  ...
  for(j in 1:q){
    ...
    if(runcontent){
      ...
      specific.end.time = Sys.time()
      cur.specific.time = specific.end.time - specific.start.time
      specific.total.time = specific.total.time + cur.specific.time
      
    }
    
    start.gbat.time = Sys.time()
    tiss_inds=rownames(fold_hom_expr_mat)[which(!is.na(fold_hom_expr_mat[,j]))]
    tiss_response=fold_hom_expr_mat[tiss_inds,j]
    ...
    tiss_betas[[j]]<-tiss_beta_vals
    tiss_ints[[j]]<-tiss_tiss_int
    end.gbat.time = Sys.time()
    cur.gbat.time = end.gbat.time - start.gbat.time
    gbat.total.time = gbat.total.time + cur.gbat.time
...
end.time.content.universal = Sys.time()
time.taken.total.content <- end.time.content.universal-start.time.content.universal
print("time taken universal setup")
print(time.taken.univeral.setup)
print("time taken total content")
print(time.taken.total.content)
print("time taken shared")
print(shared.total.time)
print("time taken specific")
print(specific.total.time)
print("time taken gbat")
print(gbat.total.time)
